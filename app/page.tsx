    ...
    <div className="space-y-20">
      ...
      <section className="grid gap-6 md:gap-8">
        ...
        <p className="max-w-2xl text-lg text-slate-600 dark:text-slate-300">
          Every hadith carries its collection and number. Every ayah links to its surah and verse.
          Built for readers who want to verify before they believe.
        </p>
        <div className="flex flex-wrap gap-3 pt-2">
          <Link
            href="/sirah"
            className="inline-flex items-center rounded-full bg-brand-500 text-white px-5 py-2.5 text-sm font-medium hover:bg-brand-600"
          >
            Read the sirah →
          </Link>
          <Link
            href="/duas"
            className="inline-flex items-center rounded-full border border-slate-300 dark:border-slate-700 px-5 py-2.5 text-sm font-medium hover:border-brand-500 hover:text-brand-500"
          >
            Browse duas
          </Link>
        </div>
      ...
      {latest.length > 0 && (
        <section>
          <div className="flex items-baseline justify-between mb-6">
            ...
            <Link href="/sirah" className="text-sm text-brand-500 hover:text-brand-600">
              All posts →
            </Link>
          </div>
          <ul className="grid gap-2 md:grid-cols-2">
            {latest.map((p) => (
              <li key={p.slug}>
                <Link
                  href={p.url}
                  className="block rounded-xl border border-slate-200 dark:border-slate-800 p-5 hover:border-brand-500 group h-full"
                >
                  <span className="text-[11px] uppercase tracking-wider text-brand-500 font-medium">
                    {p.cluster.replace(/-/g, " ")}
                  </span>
                  <h3 className="mt-2 text-base font-medium group-hover:text-brand-500">
                    {p.title}
                  </h3>
                  {p.description && (
      ...
      <section>
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Ten focused clusters. Each post grounded in primary sources.
          </p>
        </div>
      ...
      <section className="rounded-2xl bg-gradient-to-br from-brand-500 to-brand-700 text-white p-8 md:p-12">
        <div className="max-w-2xl space-y-4">
          <h2 className="text-2xl md:text-3xl font-display font-semibold tracking-tight">
            Ask anything about the Prophet ﷺ
          </h2>
          <p className="text-white/85">
            The chatbot answers only from authentic sources — Quran, Bukhari, Muslim, the four Sunan,
            and the articles on this site. Five questions a day, free. Unlimited with any donation.
          </p>
          <p className="text-sm text-white/70">
            Look for the green button in the bottom-right of any page.
          </p>
        </div>
      </section>
    </div>
